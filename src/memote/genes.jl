...
function unannotated_genes(
    ...
    annotation_keywords = ["kegg.genes", "refseq", "uniprot", 
    "ecogene", "ncbigi", "ncbigene", "ncbiprotein", "ccds", "hprd", "asap"]
  )
...
function gene_annotation_conformity(
    model::MetabolicModel;
    GENE_PRODUCT_ANNOTATIONS = Dict("refseq" => r"^((AC|AP|NC|NG|NM|NP|NR|NT|NW|XM|XP|XR|YP|ZP)_\d+|(NZ\_[A-Z]{4}\d+))(\.\d+)?$",
    "uniprot" => r"^([A-N,R-Z][0-9]([A-Z][A-Z, 0-9][A-Z, 0-9][0-9]){1,2})|([O,P,Q][0-9][A-Z, 0-9][A-Z, 0-9][A-Z, 0-9][0-9])(\.\d+)?$",
    "ecogene" => r"^EG\d+$", 
    "kegg.genes" => r"^\w+:[\w\d\.-]*$", 
    "ncbigi" => r"^(GI|gi)\:\d+$", 
    "ncbigene" => r"^\d+$", 
  ...
  for gid in genes(model) 
    ...
    for anno_kw in keys(GENE_PRODUCT_ANNOTATIONS)
      if haskey(annos, anno_kw) == true 
        ...
        in.(nothing, Ref(match.(GENE_PRODUCT_ANNOTATIONS[anno_kw], annos[anno_kw]))) && push!(no_annos[anno_kw], gid)
      end  
    end
  end
  return no_annos
